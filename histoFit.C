...
void histoFit(){ //main
  ...
  TFile *gammaTemp = new TFile("gammaTemplate.root");
  TFile *hadronTemp = new TFile("hadronicTemplate.root");
  ...
  for(unsigned ibin(2); ibin<16;ibin++)
    {
      ...
      imageName. Form("test_result/Pt_bin(%d,%d).png",binrange[ibin],binrange[ibin+1]);
  ...
  c1->SaveAs("test_result/HadronFraction.png");
  //delete c1;
